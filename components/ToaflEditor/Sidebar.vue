...
<template>
  <div>
    ...
    <!-- Sidebar Navigation -->
    <nav class="w-64 bg-white border-r border-gray-200 p-4 fixed h-full z-30 md:static transform transition-transform duration-300 ease-in-out flex flex-col"
         :class="isOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'">
      
      <div class="flex-grow">
        <h3 class="font-semibold text-gray-500 mb-3 text-sm tracking-wider uppercase px-2">Bagian Ujian</h3>
        <div class="space-y-1">
      ...
      <button @click="emit('add-section')" class="mt-4 w-full py-2.5 rounded-lg bg-indigo-100 text-indigo-700 font-semibold hover:bg-indigo-200 focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-all duration-200 flex items-center justify-center space-x-2">
